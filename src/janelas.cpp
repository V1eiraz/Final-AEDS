...
void JanelasTemporais::registrar(uint32_t id_termo, uint32_t indice_manchete) {
    // Janela calculada diretamente do índice: sem precisar saber o total antecipadamente.
    // A última janela absorve o excedente via clamp — sem overflow, sem segunda passagem.
    const uint32_t j = std::min(indice_manchete / TAMANHO_JANELA,
                                static_cast<uint32_t>(TOTAL_JANELAS - 1));
    janelas[j][id_termo]++;
    global[id_termo]++;
}
...
std::vector<std::pair<uint32_t, uint32_t>> JanelasTemporais::top_global(uint32_t n) const {
    ...
    std::partial_sort(pares.begin(), pares.begin() + k, pares.end(),
        [](const Par& a, const Par& b) { return a.second > b.second; });
    pares.resize(k);
    return pares;
}
...
std::vector<TermoEmergente> JanelasTemporais::top_emergentes(uint32_t n) const {
    ...
    for (const auto& par : global) {
        ...
        const float taxa = static_cast<float>(
            (static_cast<int64_t>(fj5) - static_cast<int64_t>(fj1))
        ) / (fj1 + 1.0f);
    ...
    std::partial_sort(emergentes.begin(), emergentes.begin() + k, emergentes.end(),
        [](const TermoEmergente& a, const TermoEmergente& b) { return a.taxa > b.taxa; });
    emergentes.resize(k);
    return emergentes;
}